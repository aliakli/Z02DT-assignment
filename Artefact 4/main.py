...
class CalcLogs:
    ...
    def add_to_db(self, user, expression, mode, error=None):
        self.UM.add_calculation_logs(user, expression, mode, errors=error)  # Store log
...
class CalculatorApp:
    def __init__(self, root, user, isAdmin):
        ...
        # Mode Label 
        self.mode_label = tk.Label(root, text=f"Mode: {self.mode}", font=("Arial", 12))
        ...
        self.scrollbar = tk.Scrollbar(output_frame)  # Scrollbar
        self.scrollbar.pack(side=tk.RIGHT, fill=tk.Y)

        self.output = tk.Text(output_frame, height=15, width=75,
                              state="disabled", yscrollcommand=self.scrollbar.set)
        self.output.pack(side=tk.LEFT)

        self.scrollbar.config(command=self.output.yview)  # Link scrollbar

        # -------- Input Field --------
        self.entry = tk.Entry(root, width=50)
        self.entry.pack(pady=5)
        self.entry.bind("<Return>", self.process_expression)  # Enter key calculates

        # -------- Buttons --------
        button_frame = tk.Frame(root)
        button_frame.pack(pady=10)

        tk.Button(button_frame, text="Calculate", command=self.process_expression).grid(row=0, column=0, padx=5)
        tk.Button(button_frame, text="Switch Mode", command=self.toggle_mode).grid(row=0, column=1, padx=5)
        tk.Button(button_frame, text="Clear Screen", command=self.clear_screen).grid(row=0, column=2, padx=5)
        tk.Button(button_frame, text="View My History", command=self.view_user_history).grid(row=0, column=3, padx=5)
        tk.Button(button_frame, text="Exit", command=self.exit_program).grid(row=0, column=4, padx=5)

    # Write to output 
    def write_output(self, text):
        self.output.config(state="normal")      # Enable temporarily
        self.output.insert(tk.END, text)        # Insert text
        self.output.see(tk.END)                 # Auto-scroll
        self.output.config(state="disabled")    # Disable again

    # Toggle Degrees/Radians
    def toggle_mode(self):
        self.instance.angle_mode = not self.instance.angle_mode
        self.mode = "Radians" if self.instance.angle_mode else "Degrees"
                ...
                for row in reader:
                    ...
                    if int(userid) == self.userid:
                        ...
                        if errors:
                            ...
                            self.write_output(f"{expression} | {mode} | {timestamp}\n")

                if not found:
                    self.write_output("No history found for this user.\n")

                self.write_output("--- End of History ---\n\n")

        except FileNotFoundError:
            self.write_output("No log file found. Please exit once to generate it.\n")
    ...
    def process_expression(self, event=None):
        ...
        try:
            result = self.instance.tokenise_expression(expression.replace("^", "**"))
            self.write_output(f"{expression} = {result}\n")
            self.logs.add_to_db(self.user, f"{expression}={result}", self.mode)

        except Exception as err:
            if len(expression) > 0 and not expression[0].isdigit():
                self.write_output("Invalid syntax\n")
                self.logs.add_to_db(self.user, expression, self.mode, error="Syntax Error")
            else:
                self.write_output(f"Error: {str(err)}\n")
                self.logs.add_to_db(self.user, expression, self.mode, error=str(err))

    # Exit 
    def exit_program(self):
        self.logs.export()   # Export logs to CSV
        self.root.destroy()  # Close window


# ================= MAIN FUNCTION =================
def main():
    login = Login()  # Run login system
